...
function image_feats = fisher_sifts(image_paths)
...
for i = 1:numel(image_paths)
    ...
    ENC = vl_fisher(double(sift_feat), means, covariances, priors, 'Normalized');
    image_feats(i,:) = ENC';
...
%[coeff, image_feats, latent] = pca(image_feats, 'NumComponents', 50);
...
% image_feats = image_feats ./ repmat(sqrt(var_row),1,feat_size);

...
%0.00088 = 0.557
%0.00001 = 0.517
fprintf('SVM clasification\n');
categories = unique(train_labels); 
num_categories = length(categories);
test_values = zeros(num_categories, size(test_image_feats,1));

for i = 1:num_categories
    current_lables = strcmp(categories(i),train_labels(:,:));
    current_lables = double(current_lables);
    [I,J] = find(current_lables == 0);
    current_lables(I,J) = -1;    
   
    [W B] = vl_svmtrain(double(train_image_feats)', current_lables, lambda);
    test_values(i,:) = (W' * test_image_feats') + repmat(B,1, size(test_image_feats,1));
    
end

[max_val, max_ind] = max(test_values, [], 1);
predicted_categories = categories(max_ind);

...
% Feature recognition, and classification code implemented by Anisha Gartia
...
switch lower(FEATURE)    
    ...
    case 'bag of gist'
        ...
        % YOU CODE get_bags_of_gist.m
        train_image_feats = get_bags_of_GIST(train_image_paths);
        test_image_feats  = get_bags_of_GIST(test_image_paths);
        
    case 'kernel codebook encoding'
        % YOU CODE build_vocabulary.m
        if ~exist('vocab.mat', 'file')
            fprintf('No existing visual word vocabulary found. Computing one from training images\n')            
            vocab = build_vocabulary(train_image_paths, vocab_size);
        ...
        % YOU CODE get_bags_of_sifts.m
        train_image_feats = kernel_get_bags_of_sifts(train_image_paths);
        test_image_feats  = kernel_get_bags_of_sifts(test_image_paths);        

    case 'fisher sift'
        
        if ~exist('means.mat', 'file')
            fprintf('No existing GMM found. Computing one from training images\n')
            num_clusters = 40; %Larger values will work better (to a point) but be slower to compute
            [means, covariances, priors] = fisher_build_vocabulary(train_image_paths, num_clusters);
        ...
        % YOU CODE get_bags_of_sifts.m
        train_image_feats = fisher_sifts(train_image_paths);
        test_image_feats  = fisher_sifts(test_image_paths); 
        
    case 'combined spatial fisher gist'
        
        if ~exist('means.mat', 'file')
            fprintf('No existing GMM found. Computing one from training images\n')
            num_clusters = 50; %Larger values will work better (to a point) but be slower to compute
            [means, covariances, priors] = fisher_build_vocabulary(train_image_paths, num_clusters);
            save('means.mat', 'means')
            save('covariances.mat', 'covariances')
            save('priors.mat', 'priors')
...
% and test on many crandom splits of the data. You are not required to do so
% for this pro ject.

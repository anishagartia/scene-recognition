...
N = 1; % Number of nearest neighbours to be considered
D = vl_alldist2(test_image_feats',train_image_feats');
[Y,I] = sort(D,2,'ascend');
nn_lables = train_labels(I(:,1:N));
%predicted_categories = mode(nn_lables,2);

for i = 1:numel(categories)
    count_cat(:,i) = sum(strcmp(categories(i),nn_lables(:,:)),2);
end

[Y2, I2] = max(count_cat,[],2);

...
function image_feats = get_bags_of_GIST(image_paths)
...
load('vocab.mat')
%vocab = [vocab_gist; vocab];
vocab_size = size(vocab, 1);

image_feats = zeros(numel(image_paths), vocab_size+128);
...
for i = 1:numel(image_paths)
    ...
    %image_feats(i,:) = image_feats(i,:) + gist;
    image_feats(i,:) = [N, gist];
    
    %all_dist = vl_alldist2(double(gist'), vocab');
    %[Y,I] = min(all_dist,[],2);
    %[N, edges] = histcounts(I,1:vocab_size+1);
    %image_feats(i,:) = image_feats(i,:) + N;
end
...
feat_size = size(image_feats,2);
image_feats = image_feats - repmat(mean_row, 1, feat_size);
var_row = var(image_feats,0,2);
image_feanats = image_feats ./ repmat(sqrt(var_row),1,feat_size);

% norm_two = norm(image_feats);
% image_feats = image_feats ./ norm_two;
